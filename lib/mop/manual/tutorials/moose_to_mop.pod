...
the slot is undef. This is specifically there to support the 
versions of C<clearer> and C<predicate> that we detail below.
Additionally this is more in line with how regular scalars work,
meaning that if you define a scalar with C<my $foo;> then it 
is implictly undef and not some special "not assigned too" value.
...
and just call a method. Here is an example:
    ...
    class Foo {
        has $bar = ${^SELF}->build_bar;

        method build_bar { ... }
    }

The C<${^SELF}> value is a special global value that will always 
contain the current invocant (when it is in scope).
...
NOTE: Within this prototype you must use the C<mop::defined>
function instead of the built-in C<defined> function. This 
is specifically a limitation of the prototype.
