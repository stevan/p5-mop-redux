...
;; With modifications from Fabrice Gabolde for p5-mop syntax support.
...
(defvar perl-imenu-generic-expression
  '(;; Functions
    (nil "^\\s-*\\(?:sub\\|method\\)\\s-+\\([-A-Za-z0-9+_:]+\\)" 1)
    ;;Variables
    ("Variables" "^\\(?:my\\|our\\)\\s-+\\([$@%][-A-Za-z0-9+_:]+\\)\\s-*=" 1)
    ("Packages" "^\\(?:package\\|class\\)\\s-+\\([-A-Za-z0-9+_:]+\\);" 1)
...
(defconst perl-font-lock-keywords-1
  '(;; What is this for?
    ...
    ;;  (1 font-lock-constant-face) (2 font-lock-variable-name-face nil t))
    ;;
    ;; Fontify function and package names in declarations.  Role
    ;; imports and subclassing are fontified like regular imports.
    ("\\<\\(class\\|package\\|method\\|sub\\)\\>[ \t]*\\(\\(::\\)?\\sw+\\(::\\sw+\\)*\\)?"
     (1 font-lock-keyword-face) (2 font-lock-function-name-face nil t))
    ("\\<\\(import\\|no\\|require\\|use\\|extends\\|with\\)\\>[ \t]*\\(\\sw+\\(::\\sw+\\)*\\)?"
...
(defconst perl-font-lock-keywords-2
  (append perl-font-lock-keywords-1
   (list
    ...
    (concat "\\<"
	    (regexp-opt '("if" "until" "while" "elsif" "else" "unless" "given" "when"
			  "do" "dump" "for" "foreach" "exit" "die" "has"
  ...
  (let ((case-fold-search nil))
    ...
    (funcall
     (syntax-propertize-rules
      ...
      ("\\<\\(?:sub\\|method\\)\\(?:[[:space:]]+[^{}[:punct:][:space:]]+\\)?[[:space:]]*(\\([^)]+\\))"
       (1 "."))
...
(defun perl-outline-level ()
  (cond
   ((looking-at "\\(?:package|class\\)\\s-") 0)
   ((looking-at "\\(?:sub\\|method\\)\\s-") 1)
